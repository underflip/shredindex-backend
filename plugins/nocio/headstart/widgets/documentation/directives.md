...
Return an October CMS component

```graphql
type Query {
   post: BlogPost @component(alias: "blogPost")
}
```

If no alias is provided, the field name is used. The directive will try to call a resolve method ``resolve<FieldName>``
of the component or return the component object itself if no resolve method is present. 
You can also specify the resolver method name explicitly.

```graphql
type Query {
   post: BlogPost @component(method: "post")
}
```
...
in the default model namespace `App\Models`.
...
the built in directives for querying data and mutating data,
as well as the convention based approach to implementing custom resolvers.
...
of the default namespaces that are defined in the configuration. The following
...
Other than global middleware defined in the configuration, field middleware
